...
Shader "iwsd/Panorama_SingleCamera" {
	...
	Tags {
		...
		float fovInRad = _FovInDegrees * UNITY_PI / 180.0;
                ...
                fixed4 col
		    = Project2Tex(direction, fovInRad, _MainTex, _OutOfRangeColor);
...
// comment
// direction = mul(unity_WorldToObject, direction);で解決した？
// unity_WorldToObject だとScaleが反映されてしまう
// fixed3x3(normalize(unity_WorldToObject[0].xyz), normalize(unity_WorldToObject[1].xyz), normalize(unity_WorldToObject[2].xyz))に